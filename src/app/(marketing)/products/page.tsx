              ...
              <ul className="grid gap-3 sm:grid-cols-2">
                {CATEGORIES.map((cat, idx) => (
                  <li key={cat.key}>
                    <Link
                      href={`#${cat.anchor}`}
                      className="group flex items-center justify-between rounded-xl border border-line bg-white px-4 py-3 text-sm font-semibold text-ink transition hover:border-blue-700/30 hover:bg-blue-700/5"
                    >
                      <span className="inline-flex items-center gap-2">
                        <span className="inline-flex size-7 items-center justify-center rounded-md bg-blue-700/5 text-xs font-bold text-blue-700">
                          {String.fromCharCode(65 + idx)}
                        </span>
                        {cat.title}
                      </span>
                      <ArrowRight className="size-4 text-blue-700 transition-transform group-hover:translate-x-0.5" />
                    </Link>
                  </li>
                ))}
              </ul>
