...
const DEFAULT_CONFIG: LoanConfigData = {
  valuationFee: 1500,
  legalFee: 1500,
...
export function SimulatorPageContent() {
  ...
  const [loanConfig, setLoanConfig] = useState<LoanConfigData>(DEFAULT_CONFIG);
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Calculate fees
      const processingFee = Math.round(takeHomeNum * loanConfig.processingFeePercentage);
      const feesTotal =
        ...
        {step === "input" && (
          <div className="space-y-6 sm:space-y-8">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-peach text-indigo font-semibold hover:brightness-95 active:brightness-90 py-6 text-base rounded-lg border-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              style={{
